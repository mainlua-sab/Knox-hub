...
local function KnoxApplyCustomSky(mode)
    ...
    if not preset or preset.kind=="off" then 
        ...
        -- Recreate default sky
        local defaultSky = Instance.new("Sky")
        defaultSky.StarCount=3000
        defaultSky.MoonAngularSize=11
        defaultSky.SunAngularSize=21
        defaultSky:SetAttribute(KNOX_SKY_TAG,true)
...
Logo.ZIndex=103
Logo.Parent=Hdr
Corner(Logo,6)local TitleL=Instance.new("TextLabel")
...
local function createSettingsGui()
    ...
    -- On/Off Toggle for Sky Themes
    local skyToggleRow = Instance.new("Frame", scroll)
    skyToggleRow.Size = UDim2.new(1, 0, 0, 32)
    skyToggleRow.BackgroundColor3 = T.Card
    skyToggleRow.BorderSizePixel = 0
    skyToggleRow.ZIndex = 202
    skyToggleRow.LayoutOrder = #scroll:GetChildren()
    Corner(skyToggleRow, 6)
    Stroke(skyToggleRow, T.Border, 0.5)

    local skyToggleLbl = Instance.new("TextLabel", skyToggleRow)
    skyToggleLbl.Size = UDim2.new(0.6, 0, 1, 0)
    skyToggleLbl.Position = UDim2.new(0, 10, 0, 0)
    skyToggleLbl.BackgroundTransparency = 1
    skyToggleLbl.Text = "Sky Themes"
    skyToggleLbl.TextSize = 10
    skyToggleLbl.Font = Enum.Font.GothamBold
    skyToggleLbl.TextColor3 = T.White
    skyToggleLbl.TextXAlignment = Enum.TextXAlignment.Left
    skyToggleLbl.ZIndex = 203

    local skyTogglePill = Instance.new("Frame", skyToggleRow)
    skyTogglePill.Size = UDim2.new(0, 34, 0, 18)
    skyTogglePill.Position = UDim2.new(1, -44, 0.5, -9)
    skyTogglePill.BackgroundColor3 = (currentSkyTheme ~= "Off") and T.Green or T.Dim
    skyTogglePill.BorderSizePixel = 0
    skyTogglePill.ZIndex = 203
    Corner(skyTogglePill, 9)

    local skyToggleDot = Instance.new("Frame", skyTogglePill)
    skyToggleDot.Size = UDim2.new(0, 12, 0, 12)
    skyToggleDot.Position = (currentSkyTheme ~= "Off") and UDim2.new(1, -15, 0.5, -6) or UDim2.new(0, 3, 0.5, -6)
    skyToggleDot.BackgroundColor3 = T.White
    skyToggleDot.BorderSizePixel = 0
    skyToggleDot.ZIndex = 204
    Corner(skyToggleDot, 6)

    local skyToggleBtn = Instance.new("TextButton", skyTogglePill)
    skyToggleBtn.Size = UDim2.new(1, 0, 1, 0)
    skyToggleBtn.BackgroundTransparency = 1
    skyToggleBtn.Text = ""
    skyToggleBtn.ZIndex = 205

    local skyEnabled = (currentSkyTheme ~= "Off")
    local selectedTheme = currentSkyTheme

    skyToggleBtn.MouseButton1Click:Connect(function()
        skyEnabled = not skyEnabled
        Tw(skyTogglePill, TweenInfo.new(0.15), {BackgroundColor3 = skyEnabled and T.Green or T.Dim})
        Tw(skyToggleDot, TweenInfo.new(0.15, Enum.EasingStyle.Back), {
            Position = skyEnabled and UDim2.new(1, -15, 0.5, -6) or UDim2.new(0, 3, 0.5, -6)
        })
        if not skyEnabled then
            selectedTheme = "Off"
            themeDD.Text = "Off"
            KnoxApplyCustomSky("Off")
            config.skyTheme = "Off"
            saveConfig()
        end
    end)
    ...
    -- Theme Dropdown
    local themeRow = Instance.new("Frame", scroll)
    themeRow.Size = UDim2.new(1, 0, 0, 32)
    themeRow.BackgroundColor3 = T.Card
    themeRow.BorderSizePixel = 0
    themeRow.ZIndex = 202
    themeRow.LayoutOrder = #scroll:GetChildren()
    Corner(themeRow, 6)
    Stroke(themeRow, T.Border, 0.5)

    local themeLbl = Instance.new("TextLabel", themeRow)
    themeLbl.Size = UDim2.new(0.3, 0, 1, 0)
    themeLbl.Position = UDim2.new(0, 10, 0, 0)
    themeLbl.BackgroundTransparency = 1
    themeLbl.Text = "Theme"
    themeLbl.TextSize = 10
    themeLbl.Font = Enum.Font.GothamBold
    themeLbl.TextColor3 = T.White
    themeLbl.TextXAlignment = Enum.TextXAlignment.Left
    themeLbl.ZIndex = 203

    local themeDD = Instance.new("TextButton", themeRow)
    themeDD.Size = UDim2.new(0, 100, 0, 22)
    themeDD.Position = UDim2.new(1, -110, 0.5, -11)
    themeDD.BackgroundColor3 = T.Card
    themeDD.BorderSizePixel = 0
    themeDD.Text = currentSkyTheme
    themeDD.TextSize = 9
    themeDD.Font = Enum.Font.GothamBold
    themeDD.TextColor3 = T.Accent
    themeDD.ZIndex = 203
    Corner(themeDD, 5)
    Stroke(themeDD, T.Border, 0.5)

    local themeIdx = 1
    for i, v in ipairs(SkyOrder) do if v == currentSkyTheme then themeIdx = i break end end

    themeDD.MouseButton1Click:Connect(function()
        themeIdx = themeIdx % #SkyOrder + 1
        selectedTheme = SkyOrder[themeIdx]
        themeDD.Text = selectedTheme
    end)

    -- Apply Button
    local applyRow = Instance.new("Frame", scroll)
    applyRow.Size = UDim2.new(1, 0, 0, 32)
    applyRow.BackgroundColor3 = T.Card
    applyRow.BorderSizePixel = 0
    applyRow.ZIndex = 202
    applyRow.LayoutOrder = #scroll:GetChildren()
    Corner(applyRow, 6)
    Stroke(applyRow, T.Border, 0.5)

    local applyBtn = Instance.new("TextButton", applyRow)
    applyBtn.Size = UDim2.new(1, -20, 0, 24)
    applyBtn.Position = UDim2.new(0, 10, 0.5, -12)
    applyBtn.BackgroundColor3 = T.Accent
    applyBtn.BorderSizePixel = 0
    applyBtn.Text = "Apply"
    applyBtn.TextSize = 10
    applyBtn.Font = Enum.Font.GothamBlack
    applyBtn.TextColor3 = T.BG
    applyBtn.ZIndex = 203
    Corner(applyBtn, 5)

    applyBtn.MouseButton1Click:Connect(function()
        if skyEnabled and selectedTheme ~= "Off" then
            currentSkyTheme = selectedTheme
            config.skyTheme = selectedTheme
            KnoxApplyCustomSky(selectedTheme)
            saveConfig()
        ...
        elseif not skyEnabled then
            applyBtn.Text = "Turn On First!"
            applyBtn.BackgroundColor3 = T.Yellow
            task.delay(1.5, function()
                applyBtn.Text = "Apply"
                applyBtn.BackgroundColor3 = T.Accent
            end)
    ...
    local function applyScale(newScale)
        currentScale = math.clamp(newScale, 0.7, 1.3)
